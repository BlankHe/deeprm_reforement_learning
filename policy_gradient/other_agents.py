...
def get_packer_action(machine, job_slot):
        align_score = 0
        act = [len(job_slot[0].slot) for i in range(len(machine))]
        # if no action available, hold
        for m, mach in enumerate(machine):
            for i in range(len(job_slot[m].slot)):
                new_job = job_slot[m].slot[i]
                if new_job is not None and (new_job.enter_time < mach.curr_time):  # there is a pending job
                    avbl_res = mach.avbl_slot[:new_job.len, :]
...
def get_sjf_action(machine, job_slot):
        sjf_score = 0
        act = [len(job_slot[0].slot) for i in range(len(machine))]  # if no action available, hold
        for m, mach in enumerate(machine):
            for i in range(len(job_slot[m].slot)):
                new_job = job_slot[m].slot[i]
                if new_job is not None and (new_job.enter_time < mach.curr_time):  # there is a pending job
...
def get_packer_sjf_action(machine, job_slot, knob):  # knob controls which to favor, 1 to packer, 0 to sjf
        ...
        act = [len(job_slot[0].slot) for i in range(len(machine))]  # if no action available, hold
        for m, mach in enumerate(machine):
            for i in range(len(job_slot[0].slot)):
                new_job = job_slot[0].slot[i]
                if new_job is not None and (new_job.enter_time < mach.curr_time):  # there is a pending job
...
def get_random_action(job_slot):
    num_act = len(job_slot[0].slot) + 1  # if no action available,
    act = []
    for i in range(len(job_slot)):
        act.append(np.random.randint(num_act))
    return act
